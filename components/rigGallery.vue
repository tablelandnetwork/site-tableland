<template>
  <section class="rig-gallery">
    <div class="filters text-black" data-aos="fade-up">
      ...
      <span class="filter" v-bind:class="{ active: currentFilter === 'Edge' }" v-on:click="setFilter('Edge')">Edge Riders</span>
    ...
    <div class="flex flex-wrap py-0 rig-garage">
     <div class="w-1/2 xl:w-1/4 lg:w-1/3 md:w-1/3 sm:w-1/2 px-3 py-3 rigs" v-for="(rig, index) in rigsMeta.rigs" :key="index" data-aos="fade-up" v-if="currentFilter === rig.attributes[1].value || currentFilter === 'All'">
      ...
      <h2 class="text-black font-Orbitron text-xl px-3 py-3">RIG ID #00{{ rig.id }}</h2>
      <p class="text-black px-3 py-0">{{ rig.attributes[1].value }}</p>
      <p class="text-black px-3 py-3 pb-3" :class="' rarity-' + rig.attributes[0].value">{{ rig.attributes[0].value }}/100</p>
       </a>
     </div>
   </div>
