    ...
    <div class="flex flex-wrap py-0 rig-garage">
     <div class="w-1/2 xl:w-1/4 lg:w-1/3 md:w-1/3 sm:w-1/2 lg:px-3 lg:py-3 rigs my-2" v-for="rig in rigsMeta" v-show="currentFilter === 'ALL' || currentFilter === rig.json_build_object.attributes[1].value || currentFilter === rig.json_build_object.attributes[2].value">
      ...
      <div v-if="rig.json_build_object.attributes[2].value == '1.000000'">
        ...
        <p class="text-black px-3 lg:py-3 lg:pb-3 text-bold" :class="' rarity-' + rig.json_build_object.attributes[2].value">Original: {{ rig.json_build_object.attributes[0].value }}</p>
      </div>
      <div v-else>
        <p class="text-black px-3 py-0">Fleet: {{ rig.json_build_object.attributes[1].value }}</p>
        <p class="text-black px-3 lg:py-3 lg:pb-3">Original: {{ rig.json_build_object.attributes[0].value }}%</p>
      </div>
      </a>
     </div>
   </div>
