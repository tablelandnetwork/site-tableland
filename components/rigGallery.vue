<template>
  <section class="rig-gallery">
    <div class="filters hidden lg:block text-black text-l" data-aos="fade-up">
      ...
      <span
        class="filter"
        v-bind:class="{ active: currentFilter === '1.000000' }"
        v-on:click="setFilter('1.000000')"
        >Originals</span
      >
    </div>
    ...
    <div class="flex flex-wrap py-0 rig-garage">
      <div
        ...
        v-show="
          currentFilter === 'ALL' ||
          currentFilter === rig.json_build_object.attributes[1].value ||
          currentFilter === rig.json_build_object.attributes[2].value
        "
      >
        <a :href="'/rigs/' + rig.json_build_object.name.replace('#', '')">
          <div
            class="rig-frame"
            :class="rig.json_build_object.attributes[1].value"
          >
            <img :src="rig.json_build_object.thumb" />
          </div>
          <h2 class="text-black font-Orbitron text-l lg:text-xl px-3 lg:py-3">
            RIG ID {{ rig.json_build_object.name }}
          </h2>

          <div v-if="rig.json_build_object.attributes[2].value == '1.000000'">
            <p class="text-black px-3 py-0">
              Fleet: {{ rig.json_build_object.attributes[3].value }}
            </p>
            <p
              class="text-black px-3 lg:py-3 lg:pb-3 text-bold"
              :class="' rarity-' + rig.json_build_object.attributes[2].value"
            >
              1.0/1.0 {{ rig.json_build_object.attributes[0].value }}
            </p>
          </div>
          <div v-else>
            <p class="text-black px-3 py-0">
              Fleet: {{ rig.json_build_object.attributes[1].value }}
            </p>
            <p class="text-black px-3 lg:py-3 lg:pb-3">
              Original: {{ rig.json_build_object.attributes[0].value }}%
            </p>
          </div>
        </a>
      </div>
